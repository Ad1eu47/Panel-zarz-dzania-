...
async function alerts(env:Env){
 ...
 for(const e of ev.results){
  ...
  const role=e.discord_role_id?'<@&'+e.discord_role_id+'> ':'';
  const text=role+'📅 **'+e.title+'** za około **'+th+'h**.\\nTermin: '+new Date(e.starts_at).toLocaleString('pl-PL',{timeZone:env.TIMEZONE});
  const msg=await discordSend(env,text,hook);
