...
if(p==='/api/bootstrap'&&req.method==='GET'){await ensureEventSettings(env);await env.DB.prepare("INSERT OR IGNORE INTO money_balances(owner,clean,dirty) SELECT character_name,0,0 FROM users WHERE character_name IS NOT NULL").run();const moneyQ=u.role==='admin'?env.DB.prepare("SELECT mb.* FROM money_balances mb WHERE mb.owner<>'Wspólne' AND EXISTS(SELECT 1 FROM users x WHERE x.character_name=mb.owner) ORDER BY mb.owner"):env.DB.prepare("SELECT * FROM money_balances WHERE owner=?").bind(u.character_name);const [assets,money,events,sales,notifications]=await Promise.all([env.DB.prepare("SELECT * FROM assets WHERE deleted_at IS NULL ORDER BY expires_at").all(),moneyQ.all(),env.DB.prepare("SELECT e.*,s.notification_category,s.reminder_hours,s.discord_role_id,COALESCE(s.enabled,0) reminder_enabled FROM calendar_events e LEFT JOIN calendar_event_settings s ON s.event_id=e.id WHERE e.deleted_at IS NULL ORDER BY e.starts_at").all(),env.DB.prepare("SELECT s.*,p.name buyer_name,p.cid FROM sales s JOIN players p ON p.id=s.player_id WHERE s.deleted_at IS NULL ORDER BY s.created_at DESC").all(),env.DB.prepare("SELECT * FROM notifications WHERE user_id IS NULL OR user_id=? ORDER BY created_at DESC LIMIT 50").bind(u.id).all()]);const moneyRows:any[]=money.results as any[];if(u.role==='admin'){const shared=moneyRows.reduce((a:any,x:any)=>({clean:a.clean+Number(x.clean||0),dirty:a.dirty+Number(x.dirty||0)}),{clean:0,dirty:0});moneyRows.push({owner:'Wspólne',clean:shared.clean,dirty:shared.dirty,computed:1})}return json({user:u,assets:assets.results,money:moneyRows,events:events.results,sales:sales.results,notifications:notifications.results},200,cors(env))}
...
if(p==='/api/admin/test-discord'&&req.method==='POST'){if(u.role!=='admin')return json({error:'FORBIDDEN'},403,cors(env));const hooks=[['Lombard',env.DISCORD_WEBHOOK_LOMBARD],['Zagrody',env.DISCORD_WEBHOOK_ZAGRODY],['Pola',env.DISCORD_WEBHOOK_POLA],['LS Motors',env.DISCORD_WEBHOOK_MOTORS],['Mechanik',env.DISCORD_WEBHOOK_GARAGE],['Umowy LS Motors',env.DISCORD_WEBHOOK_CONTRACTS],['Ogólny',env.DISCORD_WEBHOOK_URL]].filter((x:any)=>x[1]);if(!hooks.length)return json({error:'Brak skonfigurowanych webhooków'},400,cors(env));try{for(const [name,hook] of hooks as any[])await discordSend(env,'✅ **Holdings Panel** — test webhooka: **'+name+'**.',hook);return json({ok:true,count:hooks.length},200,cors(env))}catch(e:any){return json({error:'Jeden z webhooków Discord nie odpowiedział poprawnie'},502,cors(env))}}
