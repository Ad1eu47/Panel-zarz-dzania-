...
export interface Env{DB:D1Database;DOCS:R2Bucket;APP_URL:string;TIMEZONE:string;ALERT_HOURS:string;DISCORD_CLIENT_ID:string;DISCORD_CLIENT_SECRET:string;DISCORD_REDIRECT_URI:string;DISCORD_WEBHOOK_URL?:string;DISCORD_WEBHOOK_LOMBARD?:string;DISCORD_WEBHOOK_ZAGRODY?:string;DISCORD_WEBHOOK_POLA?:string;DISCORD_WEBHOOK_MOTORS?:string;DISCORD_WEBHOOK_GARAGE?:string;DISCORD_WEBHOOK_CONTRACTS?:string;ADMIN_DISCORD_ID:string;SESSION_SECRET:string;VAPID_PUBLIC_KEY:string;VAPID_PRIVATE_KEY:string;VAPID_SUBJECT:string}
...
async function ensureAssetSettings(env:Env){
 await env.DB.prepare("CREATE TABLE IF NOT EXISTS asset_alert_settings(asset_id INTEGER PRIMARY KEY,reminder_hours TEXT NOT NULL DEFAULT '72,24,12,6,1',updated_at TEXT DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(asset_id) REFERENCES assets(id))").run()
}
...
async function alerts(env:Env){
 ...
 const {results}=await env.DB.prepare("SELECT a.*,COALESCE(cfg.reminder_hours,?) reminder_hours,s.last_threshold,s.discord_message_id FROM assets a LEFT JOIN asset_alert_settings cfg ON cfg.asset_id=a.id LEFT JOIN alert_state s ON s.asset_id=a.id WHERE a.active=1 AND a.deleted_at IS NULL").bind(env.ALERT_HOURS||'72,24,12,6,1').all<any>();
...
if(p==='/api/bootstrap'&&req.method==='GET'){await ensureEventSettings(env);await ensureAssetSettings(env);await ensureNotificationReads(env);await env.DB.prepare("INSERT OR IGNORE INTO money_balances(owner,clean,dirty) SELECT character_name,0,0 FROM users WHERE character_name IS NOT NULL").run();const moneyQ=u.role==='admin'?env.DB.prepare("SELECT mb.* FROM money_balances mb WHERE mb.owner<>'Wspólne' AND EXISTS(SELECT 1 FROM users x WHERE x.character_name=mb.owner) ORDER BY mb.owner"):env.DB.prepare("SELECT * FROM money_balances WHERE owner=?").bind(u.character_name);const [assets,money,events,sales,notifications]=await Promise.all([env.DB.prepare("SELECT a.*,COALESCE(cfg.reminder_hours,?) reminder_hours FROM assets a LEFT JOIN asset_alert_settings cfg ON cfg.asset_id=a.id WHERE a.deleted_at IS NULL ORDER BY a.expires_at").bind(env.ALERT_HOURS||'72,24,12,6,1').all(),moneyQ.all(),env.DB.prepare("SELECT e.*,s.notification_category,s.reminder_hours,s.discord_role_id,COALESCE(s.enabled,0) reminder_enabled FROM calendar_events e LEFT JOIN calendar_event_settings s ON s.event_id=e.id WHERE e.deleted_at IS NULL ORDER BY e.starts_at").all(),env.DB.prepare("SELECT s.*,p.name buyer_name,p.cid FROM sales s JOIN players p ON p.id=s.player_id WHERE s.deleted_at IS NULL ORDER BY s.created_at DESC").all(),env.DB.prepare("SELECT n.*,nr.read_at user_read_at FROM notifications n LEFT JOIN notification_reads nr ON nr.notification_id=n.id AND nr.user_id=? WHERE n.user_id IS NULL OR n.user_id=? ORDER BY n.created_at DESC LIMIT 50").bind(u.id,u.id).all()]);const moneyRows:any[]=money.results as any[];if(u.role==='admin'){const shared=moneyRows.reduce((a:any,x:any)=>({clean:a.clean+Number(x.clean||0),dirty:a.dirty+Number(x.dirty||0)}),{clean:0,dirty:0});moneyRows.push({owner:'Wspólne',clean:shared.clean,dirty:shared.dirty,computed:1})}return json({user:u,assets:assets.results,money:moneyRows,events:events.results,sales:sales.results,notifications:notifications.results},200,cors(env))}
if(p==='/api/assets'&&req.method==='POST'){await ensureAssetSettings(env);const b=await body(req);const allowed=['Pole','Zagroda','Lombard','LS Motors','Mechanik'];if(!b.name||String(b.name).length>80||!allowed.includes(b.category)||!b.expires_at||isNaN(Date.parse(b.expires_at)))return json({error:'Nieprawidłowe dane'},400,cors(env));const r=await env.DB.prepare("INSERT INTO assets(name,category,expires_at,discord_role_id,created_by) VALUES(?,?,?,?,?)").bind(b.name,b.category,b.expires_at,b.discord_role_id||null,u.id).run();const id=Number(r.meta.last_row_id);await env.DB.prepare("INSERT INTO asset_alert_settings(asset_id,reminder_hours) VALUES(?,?)").bind(id,normalizeHours(b.reminder_hours||env.ALERT_HOURS)).run();await audit(env,u,'create','asset',id,b);return json({ok:true,id},201,cors(env))}
const am=p.match(/^\/api\/assets\/(\d+)\/renew$/);if(am&&req.method==='POST'){await ensureAssetSettings(env);const id=+am[1],b=await body(req),a=await env.DB.prepare("SELECT * FROM assets WHERE id=?").bind(id).first<any>();if(!a)return json({error:'Nie znaleziono'},404,cors(env));const hours=normalizeHours(b.reminder_hours||env.ALERT_HOURS);await env.DB.batch([env.DB.prepare("INSERT INTO renewals(asset_id,old_expires_at,new_expires_at,cost,user_id) VALUES(?,?,?,?,?)").bind(id,a.expires_at,b.expires_at,Number(b.cost)||0,u.id),env.DB.prepare("UPDATE assets SET expires_at=?,discord_role_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(b.expires_at,b.discord_role_id||null,id),env.DB.prepare("INSERT INTO asset_alert_settings(asset_id,reminder_hours,updated_at) VALUES(?,?,CURRENT_TIMESTAMP) ON CONFLICT(asset_id) DO UPDATE SET reminder_hours=excluded.reminder_hours,updated_at=CURRENT_TIMESTAMP").bind(id,hours)]);const st=await env.DB.prepare("SELECT discord_message_id FROM alert_state WHERE asset_id=?").bind(id).first<any>();await discordDelete(env,st?.discord_message_id,webhookFor(env,a.category));await env.DB.prepare("DELETE FROM alert_state WHERE asset_id=?").bind(id).run();await audit(env,u,'renew','asset',id,b);return json({ok:true},200,cors(env))}
const aam=p.match(/^\/api\/assets\/(\d+)\/alerts$/);if(aam&&req.method==='PUT'){await ensureAssetSettings(env);const id=+aam[1],b=await body(req),a=await env.DB.prepare("SELECT * FROM assets WHERE id=? AND deleted_at IS NULL").bind(id).first<any>();if(!a)return json({error:'Nie znaleziono terminu'},404,cors(env));const hours=normalizeHours(b.reminder_hours||env.ALERT_HOURS),role=String(b.discord_role_id||'').trim()||null;const st=await env.DB.prepare("SELECT discord_message_id FROM alert_state WHERE asset_id=?").bind(id).first<any>();await discordDelete(env,st?.discord_message_id,webhookFor(env,a.category));await env.DB.batch([env.DB.prepare("UPDATE assets SET discord_role_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(role,id),env.DB.prepare("INSERT INTO asset_alert_settings(asset_id,reminder_hours,updated_at) VALUES(?,?,CURRENT_TIMESTAMP) ON CONFLICT(asset_id) DO UPDATE SET reminder_hours=excluded.reminder_hours,updated_at=CURRENT_TIMESTAMP").bind(id,hours),env.DB.prepare("DELETE FROM alert_state WHERE asset_id=?").bind(id)]);await audit(env,u,'update_alerts','asset',id,{reminder_hours:hours,discord_role_id:role});return json({ok:true,reminder_hours:hours,discord_role_id:role},200,cors(env))}
...
if(p==='/api/admin/test-discord'&&req.method==='POST'){if(u.role!=='admin')return json({error:'FORBIDDEN'},403,cors(env));const channels=[['Lombard','Lombard',env.DISCORD_WEBHOOK_LOMBARD],['Zagrody','Zagroda',env.DISCORD_WEBHOOK_ZAGRODY],['Pola','Pole',env.DISCORD_WEBHOOK_POLA],['Arabic Motors','LS Motors',env.DISCORD_WEBHOOK_MOTORS],['Mechanik','Mechanik',env.DISCORD_WEBHOOK_GARAGE]].filter((x:any)=>x[2]);if(!channels.length&&!env.DISCORD_WEBHOOK_CONTRACTS&&!env.DISCORD_WEBHOOK_URL)return json({error:'Brak skonfigurowanych webhooków'},400,cors(env));let sent=0;try{for(const [label,category,hook] of channels as any[]){const rows=await env.DB.prepare("SELECT * FROM assets WHERE category=? AND active=1 AND deleted_at IS NULL ORDER BY expires_at").bind(category).all<any>();const a=(rows.results||[]).find((x:any)=>{const ts=zonedEpoch(x.expires_at,env.TIMEZONE||'Europe/Warsaw');return Number.isFinite(ts)&&ts>Date.now()});if(a){await discordSend(env,assetDiscordText(a,env,undefined,true),hook)}else{await discordSend(env,'🧪 **Holdings Panel** — test kanału **'+label+'**.\\nBrak aktywnego przyszłego terminu w tej kategorii, więc nie ma roli ani odliczania do pokazania.',hook)}sent++}if(env.DISCORD_WEBHOOK_CONTRACTS){await discordSend(env,'🧪 **Holdings Panel** — test kanału umów Arabic Motors.',env.DISCORD_WEBHOOK_CONTRACTS);sent++}if(env.DISCORD_WEBHOOK_URL){await discordSend(env,'🧪 **Holdings Panel** — test webhooka ogólnego.',env.DISCORD_WEBHOOK_URL);sent++}return json({ok:true,count:sent},200,cors(env))}catch(e:any){return json({error:'Jeden z webhooków Discord nie odpowiedział poprawnie'},502,cors(env))}}
