...
function assetDiscordText(a:any,env:Env,threshold?:number,test=false){
 const expires=zonedEpoch(a.expires_at,env.TIMEZONE||'Europe/Warsaw'),unix=Math.floor(expires/1000),left=expires-Date.now(),role=a.discord_role_id?'<@&'+a.discord_role_id+'> ':'',icon=left<=6*36e5?'🚨':'⏳';
 const lines=[role+icon+' **'+a.name+'**', '**Pozostało:** '+remainingLabel(left)+' • <t:'+unix+':R>', '**Termin:** <t:'+unix+':F>'];
 if(threshold)lines.push('**Próg alertu:** '+threshold+' h');
 if(test){lines.push('🧪 *Wiadomość testowa*');if(!a.discord_role_id)lines.push('⚠️ *Brak ID roli Discord dla tego terminu, dlatego rola nie została oznaczona.*')}
 lines.push('',takeoverWarning(a.category));
 return lines.join('\\n')
}
