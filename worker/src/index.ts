...
const atm=p.match(/^\/api\/assets\/(\d+)\/test-alert$/);if(atm&&req.method==='POST'){await ensureAssetSettings(env);const id=+atm[1],a=await env.DB.prepare("SELECT a.*,cfg.webhook_slot,cfg.reminder_hours FROM assets a LEFT JOIN asset_alert_settings cfg ON cfg.asset_id=a.id WHERE a.id=? AND a.deleted_at IS NULL").bind(id).first<any>();if(!a)return json({error:'Nie znaleziono terminu'},404,cors(env));const hook=webhookForSlot(env,a.webhook_slot,a.category);if(!hook)return json({error:'Dla tego kanału nie ustawiono webhooka w Cloudflare'},400,cors(env));const st=await env.DB.prepare("SELECT discord_message_id FROM alert_state WHERE asset_id=?").bind(id).first<any>();await discordDelete(env,st?.discord_message_id,hook);const msg=await discordSend(env,assetDiscordText(a,env,undefined,true),hook);await env.DB.prepare("INSERT INTO alert_state(asset_id,last_threshold,discord_message_id,last_sent_at) VALUES(?,NULL,?,CURRENT_TIMESTAMP) ON CONFLICT(asset_id) DO UPDATE SET discord_message_id=excluded.discord_message_id,last_sent_at=CURRENT_TIMESTAMP").bind(id,msg?.id||null).run();return json({ok:true,replaced:Boolean(st?.discord_message_id)},200,cors(env))}const arm=p.match(/^\/api\/assets\/(\d+)\/trigger-alert$/);if(arm&&req.method==='POST'){await ensureAssetSettings(env);const id=+arm[1],a=await env.DB.prepare("SELECT a.*,cfg.webhook_slot,cfg.reminder_hours FROM assets a LEFT JOIN asset_alert_settings cfg ON cfg.asset_id=a.id WHERE a.id=? AND a.deleted_at IS NULL").bind(id).first<any>();if(!a)return json({error:'Nie znaleziono terminu'},404,cors(env));const hook=webhookForSlot(env,a.webhook_slot,a.category);if(!hook)return json({error:'Dla tego kanału nie ustawiono webhooka w Cloudflare'},400,cors(env));const st=await env.DB.prepare("SELECT last_threshold,discord_message_id FROM alert_state WHERE asset_id=?").bind(id).first<any>();await discordDelete(env,st?.discord_message_id,hook);const msg=await discordSend(env,assetDiscordText(a,env),hook);await env.DB.prepare("INSERT INTO alert_state(asset_id,last_threshold,discord_message_id,last_sent_at) VALUES(?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(asset_id) DO UPDATE SET discord_message_id=excluded.discord_message_id,last_sent_at=CURRENT_TIMESTAMP").bind(id,st?.last_threshold??null,msg?.id||null).run();await audit(env,u,'trigger_alert','asset',id,{webhook_slot:a.webhook_slot||null});return json({ok:true,replaced:Boolean(st?.discord_message_id)},200,cors(env))}

if(p==='/api/money'&&req.method==='POST'){const b=await body(req),owner=String(b.owner||'');if(owner==='Wspólne')return json({error:'Wspólne jest sumą wszystkich postaci i nie można go edytować ręcznie'},400,cors(env));const old=await env.DB.prepare("SELECT * FROM money_balances WHERE owner=?").bind(owner).first<any>();if(!old)return json({error:'Nieznany właściciel'},400,cors(env));await env.DB.batch([env.DB.prepare("INSERT INTO money_log(owner,old_clean,old_dirty,new_clean,new_dirty,user_id) VALUES(?,?,?,?,?,?)").bind(owner,old.clean,old.dirty,Number(b.clean)||0,Number(b.dirty)||0,u.id),env.DB.prepare("UPDATE money_balances SET clean=?,dirty=?,updated_by=?,updated_at=CURRENT_TIMESTAMP WHERE owner=?").bind(Number(b.clean)||0,Number(b.dirty)||0,u.id,owner)]);return json({ok:true},200,cors(env))}
if(p==='/api/money/undo'&&req.method==='POST'){const q=env.DB.prepare("SELECT * FROM money_log WHERE undone=0 ORDER BY id DESC LIMIT 1");const l=await q.first<any>();if(!l)return json({error:'Brak zmian'},404,cors(env));await env.DB.batch([env.DB.prepare("UPDATE money_balances SET clean=?,dirty=?,updated_by=?,updated_at=CURRENT_TIMESTAMP WHERE owner=?").bind(l.old_clean,l.old_dirty,u.id,l.owner),env.DB.prepare("UPDATE money_log SET undone=1 WHERE id=?").bind(l.id)]);return json({ok:true},200,cors(env))}
...
if(p==='/api/admin/test-discord'&&req.method==='POST'){const hooks=[['Lombard 1',env.DISCORD_WEBHOOK_LOMBARD],['Lombard 2',env.DISCORD_WEBHOOK_LOMBARD_2],['Zagrody',env.DISCORD_WEBHOOK_ZAGRODY],['Pola',env.DISCORD_WEBHOOK_POLA],['Arabic Motors',env.DISCORD_WEBHOOK_MOTORS],['Mechanik 1',env.DISCORD_WEBHOOK_GARAGE],['Mechanik 2',env.DISCORD_WEBHOOK_GARAGE_2],['Umowy Arabic Motors',env.DISCORD_WEBHOOK_CONTRACTS],['Ogólny',env.DISCORD_WEBHOOK_URL]].filter((x:any)=>x[1]);if(!hooks.length)return json({error:'Brak skonfigurowanych webhooków'},400,cors(env));try{for(const [name,hook] of hooks as any[])await discordSend(env,'🧪 **Holdings Panel**\\nTest webhooka: **'+name+'**',hook);return json({ok:true,count:hooks.length},200,cors(env))}catch{return json({error:'Jeden z webhooków Discord nie odpowiedział poprawnie'},502,cors(env))}}
if(p==='/api/admin/users'){if(req.method==='GET'){const x=await env.DB.prepare("SELECT id,discord_id,discord_name,character_name,role,status FROM users ORDER BY created_at DESC").all();return json(x.results,200,cors(env))}if(req.method==='POST'){const b=await body(req);await env.DB.prepare("UPDATE users SET character_name=?,role=?,status=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(b.character_name,b.role||'member',b.status||'active',b.id).run();return json({ok:true},200,cors(env))}}
if(p==='/api/export'){const data:any={};for(const t of ['users','assets','renewals','money_balances','money_log','calendar_events','players','sales','activity_log'])data[t]=(await env.DB.prepare('SELECT * FROM '+t).all()).results;return json(data,200,{...cors(env),'content-disposition':'attachment; filename=holdings-panel-export.json'})}
